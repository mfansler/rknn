...
)
{
   ...
   for(i in 1:r){
        ...
        fset<- sample(p, mtry);
        selected[i,]<- fset;
        aknn<- knn(train=data[dset, fset], test=data[tset, fset], cl=y[dset], k=k, algorithm=knn.algo);
...
rknnBeg<-function(data, y, k=1, r=500,
                ...
                Random.seed=NULL,   # a seed in .Random.seed format. 
...
#backward elimination in geometric fashion
{
    ...
    varlist <- list()
    acc <- numeric(ni + 1)
    rknn <- rknnSupport(data, y, k, r, fixed.partition=fixed.partition, knn.algo=knn.algo)
    ...
    for (i in 2:(ni + 1)) {
        ...
        p <- newp
        rknn <- rknnSupport(data[, vars], y, k, r, fixed.partition=fixed.partition, knn.algo=knn.algo)
